...
#        self.params = [self.Wr, self.Wv, self.b]
        ...
        def normalized_tanh(x):
            ...
            return T.tanh(x) / T.sqrt( T.sum(T.pow(T.tanh(x),2)) )
            raise NotImplementedError
            
            
        self.f = normalized_tanh
        def recurrence_cost( x_z, prior_cost, x_c, h_n):
            return prior_cost + T.maximum(0.0, 1.0 -  T.dot(x_c, h_n) + T.dot(x_z, h_n)  )
        ...
        def recurrence(n, hidden_states, hidden_sums, cost, x, r, p, wrong_ans, corr_ans):
           ...
           '''
            ...
            h_n_state = T.set_subtensor( hidden_states[n], h_n)
#            update_n = T.dot(r[n], h_n)
            update_n = hidden_sums[p[n]] + T.dot(r[n], h_n)
            update_state = T.set_subtensor( hidden_sums[ p[n] ], update_n)
            updated_cost, updates = theano.scan( fn = lambda x_z, prior_sum, x_c, h_n: prior_sum + T.maximum(0, (1 - T.dot(x_c, h_n)) + T.dot(x_z, h_n)),
                                       sequences = wrong_ans,
                                       outputs_info = T.as_tensor_variable(np.asarray(0, theano.config.floatX)),
                                       non_sequences = [corr_ans, h_n]
                                       )

#            updated_cost, updates = theano.scan(fn = recurrence_cost,
#                                       sequences = wrong_ans,
#                                       outputs_info = T.as_tensor_variable(np.asarray(0, theano.config.floatX)),
#                                       non_sequences = [corr_ans, h_n]
#                                       )
            final_cost = cost + updated_cost[-1]                    
#            final_cost = cost + result[-1]
            return h_n_state, update_state, final_cost
#            raise NotImplementedError

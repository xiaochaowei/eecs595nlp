...
#    subdata = random.sample(data, 200)
    ...
    try:
        ...
        english = dataset.get_english_train_corpus().parsed_sents()[0]
        danish = dataset.get_danish_train_corpus().parsed_sents()[0]
        korean = dataset.get_korean_train_corpus().parsed_sents()[0]
        swedish = dataset.get_swedish_test_corpus().parsed_sents()[0]
        # tp = TransitionParser.load('badfeatures.model')

        # parsed = tp.parse(testdata)
        # print parsed
        sents = [english, danish, korean, swedish]
        with open('problem1.conll', 'w') as f:
            # for p in parsed:
            for p in sents:

...
if __name__ == '__main__':
    models = ['english', 'danish', 'swedish' ]
    random.seed(1234)

    get_train_dataset_funs = [\
        random.sample(dataset.get_english_train_corpus().parsed_sents(), 200), \
        random.sample(dataset.get_danish_train_corpus().parsed_sents(), 200), \
        random.sample(dataset.get_swedish_train_corpus().parsed_sents(), 200), \
        ]
    
    get_test_dataset_funs = [\
        dataset.get_english_test_corpus().parsed_sents(), \
        dataset.get_danish_test_corpus().parsed_sents(), \
        dataset.get_swedish_test_corpus().parsed_sents()\
        ]
    for idx, model in enumerate(models):
       	subdata = get_train_dataset_funs[idx]
    # data = dataset.get_swedish_train_corpus().parsed_sents()
        ...
        try:
            ...
            testdata = get_test_dataset_funs[idx]
